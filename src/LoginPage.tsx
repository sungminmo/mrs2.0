...
function MaterialValueGrid() {
  ...
  return <section ref={section} className="login-value-section" aria-labelledby="value-title"><div className="login-value-inner">
    <div className="login-section-heading"><div><span className="login-eyebrow">VALUE IN CIRCULATION</span><h2 id="value-title">남겨진 자재에서,<br />다시 움직이는 자산으로.</h2></div><p>무엇이 얼마나 남았는지 아는 것부터 시작합니다.<br />MRS는 보관과 거래 정보를 연결해<br />재사용과 판매를 판단할 근거를 만듭니다.</p></div>
    <ol className="login-solution-flow">
      <li><span>01 / 자산화</span><h3>흩어진 자재를 한눈에</h3><p>자재의 수량·상태·보관 위치를 기록하고 평가 가치를 확인합니다. 다시 쓸 자재와 판매할 자재를 구분할 수 있습니다.</p></li>
      <li><span>02 / 수요 연결</span><h3>보관을 넘어 다음 현장으로</h3><p>판매할 자재는 검수와 관리자 승인 후 마켓에 등록합니다. 필요한 현장의 견적 요청을 통해 거래를 시작합니다.</p></li>
      <li><span>03 / 가치 확인</span><h3>비용과 회수 금액을 함께</h3><p>로케이션별 보관 비용과 판매 정산 내역을 확인합니다. 계속 보관할지, 판매할지 판단할 정보를 모읍니다.</p></li>
    </ol>
    <h3 className="login-value-caption">자재의 흐름을 가치로 살펴보면 <span>활용 예시</span></h3>
    <dl className="login-value-grid">{valueMetrics.map((metric) => <div key={metric.label}><dt>{metric.label}</dt><dd aria-label={`${metric.value.toLocaleString('ko-KR')}원`}><span aria-hidden="true"><small>₩</small><span className="login-value-number">{Math.round(metric.value * progress).toLocaleString('ko-KR')}</span></span></dd><p>{metric.description}</p></div>)}</dl>
    <p className="login-value-note">서비스 활용을 설명하기 위한 예시 데이터입니다. 실제 이용 실적이나 수익을 보장하지 않습니다. 누적 판매 수익은 자산 가치와 별도 지표입니다.</p>
  </div></section>
}
  ...
  return <div className="login-page">
    ...
    <main className="login-main">
      ...
      <section id="material-challenges" className="login-problem-section" aria-labelledby="problem-title"><div className="login-problem-inner">
        <div className="login-section-heading"><div><span className="login-eyebrow">AFTER THE PROJECT</span><h2 id="problem-title">공사는 끝났는데,<br />남은 자재는 어디로 가나요?</h2></div><p>다시 쓸 수 있는 자재도 관리가 끊기면 짐이 됩니다.<br />현장을 정리하는 순간, 다음 고민이 시작됩니다.</p></div>
        <div className="login-problem-grid">
          <article><span>보관 공간</span><h3>일단 쌓아두면,<br />얼마나 더 보관해야 할까요?</h3><p>다음 사용 일정은 불확실한데 보관 공간은 계속 필요합니다. 어느 위치를 얼마나 점유하는지 모르면 비용을 판단하기도 어렵습니다.</p></article>
          <article><span>재고 파악</span><h3>어디에, 얼마나,<br />어떤 상태로 남아 있을까요?</h3><p>현장과 담당자마다 흩어진 기록으로는 남은 수량과 상태를 확인하기 어렵습니다. 쓸 수 있는 자재를 놓치고 새로 구매할 수도 있습니다.</p></article>
          <article><span>처분 판단</span><h3>다시 쓸 수 있는데,<br />처분만이 답일까요?</h3><p>필요한 수요처를 찾지 못하면 판매도 미뤄집니다. 자재의 가치와 보관 비용을 함께 알아야 재사용·판매·처분을 비교할 수 있습니다.</p></article>
        </div>
        <p className="login-problem-bridge">남은 자재를 없애는 일보다, <strong>다음 쓰임을 찾는 관리가 먼저입니다.</strong><ArrowDown size={20} aria-hidden="true" /></p>
      </div></section>
      <MaterialValueGrid />
      <section id="services" className="login-services" aria-labelledby="services-title"><div className="login-section-heading"><div><span className="login-eyebrow">OUR SERVICES</span><h2 id="services-title">현장의 고민을 덜어줄 세 가지 서비스</h2></div><p>보관 현황을 파악하고, 필요한 현장과 연결하고,<br />거래 이후의 정산까지 이어갑니다.</p></div><div className="login-service-grid">
        <article className="login-service-card"><div className="login-service-top"><Warehouse size={25} /><span>01 / STORAGE</span></div><h3>자재 보관·자산 관리</h3><p>흩어진 재고 기록을 자재별로 모으세요. 수량·상태와 보관 위치를 확인해 재사용과 판매 계획을 세울 수 있습니다.</p><ul><li>자재별 재고와 평가 가치</li><li>로케이션별 점유 현황</li><li>보관·판매·검수 상태 관리</li></ul><button onClick={openLogin} aria-haspopup="dialog">내 자산 관리하기<ArrowRight size={17} /></button></article>
        <article className="login-service-card"><div className="login-service-top"><ShoppingBag size={25} /><span>02 / MARKET</span></div><h3>자재 거래·마켓</h3><p>남은 자재가 필요한 현장을 만날 수 있도록 판매를 준비하세요. 구매자는 자재와 품질 등급을 비교하고 견적을 요청할 수 있습니다.</p><ul><li>검수 및 승인 후 판매 등록</li><li>품질 등급별 자재 탐색</li><li>상품·장바구니 견적 요청</li></ul><button onClick={onBrowse}>마켓 둘러보기<ArrowRight size={17} /></button></article>
        <article className="login-service-card"><div className="login-service-top"><ReceiptText size={25} /><span>03 / SETTLEMENT</span></div><h3>보관 비용·판매 정산</h3><p>보관에 든 비용과 판매로 회수한 금액을 함께 살펴보세요. 거래별 명세를 확인하고 다음 자재 운영 계획에 활용할 수 있습니다.</p><ul><li>판매 수익과 비용 내역</li><li>로케이션 단위 보관료 명세</li><li>기간별 조회와 내역 다운로드</li></ul><button onClick={openLogin} aria-haspopup="dialog">정산 관리 시작하기<ArrowRight size={17} /></button></article>
      </div></section>
