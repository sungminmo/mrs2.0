...
export default function CategorySelect({ categories, value, defaultValue = '', onChange, required = false, retainedId, name = 'category' }: Props) {
  ...
  return <fieldset className="category-select"><legend>카테고리{required ? ' (최종 분류 필수)' : ''}</legend>
    ...
    <div className="category-select-levels">{[0, 1, 2].map((depth) => {
      ...
      return <label key={depth}>{depth + 1}차 분류<select aria-label={`${depth + 1}차 분류`} required={required && children.length > 0} disabled={depth > 0 && (!parent || children.length === 0)} value={chain[depth]?.id ?? ''} onChange={(event) => update(event.target.value || parent || '')}>
