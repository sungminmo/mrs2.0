...
import { customers, dateText, inventory, invoiceAmount, invoices, masterItems, money, referenceDate } from './adminData'
...
export default function AdminPortal({ hash }: { hash: string }) {
  ...
  const views = createAdminViews(assets, items, categories)
  ...
  const editable = menu.id === 'items' || menu.id === 'inventory' && tab?.id === 'stock'
  ...
  return <div className="admin-portal">
    <header className="adm-header">
      ...
      <span className="adm-prototype">독립 예시 · 카테고리·품목·자산 임시 편집</span>
    ...
    <main className="adm-main">
      <div className="adm-heading"><div><div className="adm-breadcrumb">운영 관리 / {menu.label}{row ? ` / ${row.id}` : ''}</div><h1 ref={heading} tabIndex={-1}>{editing ? `${menu.id === 'items' ? '품목' : '자산'} ${mode === 'new' ? '등록' : '수정'}` : row ? row.title : menu.label}</h1></div><span className="adm-mode">관리자 시안</span></div>
      {menu.id === 'categories' ? <CategoryManager categories={categories} items={items} assets={assets} params={url.searchParams} onSave={(category) => { setCategories((current) => current.some((entry) => entry.id === category.id) ? current.map((entry) => entry.id === category.id ? category : entry) : [...current, category]); window.location.hash = `/admin/categories?tab=tree&id=${category.id}` }} /> : menu.id === 'dashboard' ? <Dashboard views={views} /> : <>
        ...
        {editing ? <InventoryEditor key={`${menu.id}/${mode}/${id}`} kind={menu.id === 'items' ? 'items' : 'inventory'} items={items} assets={assets} categories={categories} id={mode === 'edit' ? id : null} cancelHref={cancelHref} onSaveItem={(item) => { setItems((current) => current.some((entry) => entry.id === item.id) ? current.map((entry) => entry.id === item.id ? item : entry) : [...current, item]); saved(item.id) }} onSaveAsset={(asset) => { setAssets((current) => current.some((entry) => entry.id === asset.id) ? current.map((entry) => entry.id === asset.id ? asset : entry) : [...current, asset]); saved(asset.id) }} /> : <>
          {editable && (!id || row) && <div className="adm-management-actions"><span className="adm-note">새로고침·고객 포털 이동 시 변경 내용 초기화</span><a className="adm-button adm-primary" href={editHref}>{row ? <Pencil size={16} /> : <Plus size={16} />}{menu.id === 'items' ? '품목' : '자산'} {row ? '수정' : '등록'}</a></div>}
          {view && (id ? <><a className="adm-button adm-back" href={listHref}><ArrowLeft size={15} />목록으로</a>{row ? <RecordDetail row={row} /> : <div className="adm-empty"><h2>내역을 찾을 수 없습니다</h2><p>선택한 메뉴에 해당 번호가 없습니다.</p></div>}</> : <RecordList key={`${menu.id}/${tab?.id}`} view={view} params={url.searchParams} path={menu.id} categories={categories} />)}
        </>}
      </>}
...
function Status({ value }: { value: string }) {
  const tone = /미답변|대기|미처리|미청구|보완/.test(value) ? 'wait' : /취소|중지|미사용/.test(value) ? 'muted' : /완료|종료/.test(value) ? 'done' : 'active'
  return <span className={`adm-status adm-status-${tone}`}>{value}</span>
}

function RecordList({ view, params, path, categories }: { view: AdminView; params: URLSearchParams; path: string; categories: MaterialCategory[] }) {
  ...
  return <>
    ...
    <div className="adm-table-scroll" tabIndex={0} role="region" aria-label={`${view.title} 표`}><table><caption className="adm-sr-only">{view.title}</caption><thead><tr>{path === 'items' && <th scope="col">대표 이미지</th>}{view.headers.map((header) => <th key={header} scope="col">{header}</th>)}<th scope="col">상세</th></tr></thead><tbody>{shown.map((row) => <tr key={row.id}>{path === 'items' && <td>{row.images?.[0] ? <img className="adm-thumbnail" src={row.images[0].url} alt={`${row.title} 대표 이미지`} /> : <span className="adm-no-image" title="이미지 미등록"><ImageOff size={18} aria-label="이미지 미등록" /></span>}</td>}{row.cells.map((cell, index) => <td key={index}>{cell === row.status || cell === row.saleStatus ? <Status value={cell} /> : cell}</td>)}<td><a className="adm-detail-link" href={detailHref(row.id)} aria-label={`${row.id} 상세보기`}>상세보기<ChevronRight size={14} /></a></td></tr>)}</tbody></table></div>
